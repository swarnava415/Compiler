...
// table[i][j] stores the index of production that must be applied on ith varible if the input is jth nonterminal
int table[100][TSIZE];
char terminal[TSIZE];
char nonterminal[26];
...
int main() {
	...
	// the parse table do not read '^' as input
	// so we set terminal['^'] = 0 to remove '^' from terminals
	terminal['^'] = 0;
